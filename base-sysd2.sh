...
sed -i '/^HOOKS=/ s/udev/systemd/g' "$rootmnt"/etc/mkinitcpio.conf
sed -i '/^HOOKS=/ s/keymap/sd-vconsole/g' "$rootmnt"/etc/mkinitcpio.conf
...
# enable zswap
#echo "options rd.luks.name="$UUID"=root root=${MAPPING} rootflags=subvol=@ rd.luks.options=discard rw" >> "$rootmnt"/boot/loader/entries/arch.conf

# disable zswap
echo "options rd.luks.name="$UUID"=root root=${MAPPING} rootflags=subvol=@ rd.luks.options=discard rw zswap.enabled=0" >> "$rootmnt"/boot/loader/entries/arch.conf
